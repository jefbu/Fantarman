...
public class LeagueFixturesFixturesPanel extends ContentPanel {
	...
	public LeagueFixturesFixturesPanel(int width, int height, Colour colour, FlowLayout flowLayout) {
		...
		matchDayPanel = new ContentPanel(width / 4, height / 4, Colour.LBLUE,
		...
		matchupsPanel = new ContentPanel(width * 4 / 5, height * 2 / 3, Colour.LBLUE,
	...
	public void fillLeagueFixturesFixturesPanel() {
		
		matchDayLabeledPanel.label.setForeground(Colors.vdblue);
		matchDayLabeledPanel.setBackground(Colors.lblue);
		matchDayLabeledPanel.label.setText("Match Day: " + Main.league.calendarDay);
		
		for (MatchupRowPanel panel: matchupRowPanels) {
			panel.fillMatchupRowPanel();
		}
